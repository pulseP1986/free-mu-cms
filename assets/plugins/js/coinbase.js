...
CoinBase.prototype.setUrl = function(data){
	this.url = data;
}
CoinBase.prototype.saveSettings = function(form){
	$.ajax({
		type: "post",
		dataType: "json",
...
CoinBase.prototype.addPackage = function(){
	$.ajax({
		type: "post",
		dataType: "json",
		...
		success: function( data ){
			if(data.error){
				...
				html += '</select></td>';
				html += '<td class="center" id="status_icon_'+data.id+'"><span class="label label-success">Active</span></td><td class="center"><a class="btn btn-danger" href="#" id="status_button_'+data.id+'" onclick="CoinBase.changeStatus('+data.id+', 0);"><i class="icon-edit icon-white"></i> Disable</a>  <a class="btn btn-info" href="#" onclick="CoinBase.edit('+data.id+');"><i class="icon-edit icon-white"></i> Edit</a>  <a class="btn btn-danger" href="#" onclick="CoinBase.delete('+data.id+');"><i class="icon-trash icon-white"></i> Delete</a></td></tr>';
				$('#donate_sortable > tbody:last').append(html);
			}
		}
	});
}
CoinBase.prototype.edit = function(id){
	$.ajax({
		type: "post",
		dataType: "json",
...
CoinBase.prototype.delete = function(id){
	$.ajax({
		type: "post",
		dataType: "json",
...
CoinBase.prototype.changeStatus = function(id, status){
	$.ajax({
		type: "post",
		dataType: "json",
		...
		success: function( data ){
			if(data.error){
				...
				if(status == 1){
					...
					$('#status_button_'+id).attr('onclick', 'CoinBase.changeStatus('+id+', 0);');
				}
				else{
					...
					$('#status_button_'+id).attr('onclick', 'CoinBase.changeStatus('+id+', 1);');
				}
			}
		}
	});
}
CoinBase.prototype.saveOrder = function(){
...
CoinBase.prototype.checkout = function (id) {
	$.ajax({
