    ...
    class PagSeguroTransactionSearchService
    {
        ...
        public static function searchByCode(PagSeguroCredentials $credentials, $transactionCode)
        {
            ...
            try{
                ...
                $connection->get(self::buildSearchUrlByCode($connectionData, $transactionCode), $connectionData->getServiceTimeout(), $connectionData->getCharset());
        ...
        public static function searchByDate(PagSeguroCredentials $credentials, $pageNumber, $maxPageResults, $initialDate, $finalDate = null)
        {
            ...
            try{
                $connection = new PagSeguroHttpConnection();
                $connection->get(self::buildSearchUrlByDate($connectionData, $searchParams), $connectionData->getServiceTimeout(), $connectionData->getCharset());
        ...
        public static function searchByReference(PagSeguroCredentials $credentials, $reference, $initialDate = null, $finalDate = null, $pageNumber = null, $maxPageResults = null)
        {
            ...
            $connectionData = new PagSeguroConnectionData($credentials, self::SERVICE_NAME);
            if($initialDate)
                $searchParams = self::buildParams($pageNumber, $maxPageResults, $initialDate, $finalDate); else
                $searchParams = null;
            try{
                $connection = new PagSeguroHttpConnection();
        ...
        private function searchByCodeResult($connection, $code)
        {
            ...
            switch($httpStatus->getType()){
                case 'OK':
                    ...
                    LogPagSeguro::info("PagSeguroTransactionSearchService.SearchByCode(transactionCode=$code) - end " . $transaction->toString());
                    break;
                case 'BAD_REQUEST':
                    $errors = PagSeguroTransactionParser::readErrors($connection->getResponse());
                    $err = new PagSeguroServiceException($httpStatus, $errors);
                    LogPagSeguro::error("PagSeguroTransactionSearchService.SearchByCode(transactionCode=$code) - error " . $err->getOneLineMessage());
                    throw $err;
                    break;
                default:
