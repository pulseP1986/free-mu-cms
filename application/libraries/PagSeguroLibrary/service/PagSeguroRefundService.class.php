    ...
    class PagSeguroRefundService
    {
        ...
        private static function buildRefundURL($connectionData, $transactionCode, $refundValue = null){
            if(is_null($refundValue)){
                ...
                return $connectionData->getServiceUrl() . '?' . $connectionData->getCredentialsUrlQuery() . "&transactionCode=" . $transactionCode . "&refundValue=" . $refundValue;
            }
        }
        ...
        public static function createRefundRequest(PagSeguroCredentials $credentials, $transactionCode, $refundValue = null){
            ...
            try{
                ...
                $connection->post($url, [], $connectionData->getServiceTimeout(), $connectionData->getCharset());
                $httpStatus = new PagSeguroHttpStatus($connection->getStatus());
                switch($httpStatus->getType()){
                    case 'OK':
                        $result = PagSeguroRefundParser::readSuccessXml($connection->getResponse());
                        LogPagSeguro::info("PagSeguroRefundService.createRefundRequest(" . $result . ") - end ");
                        break;
                    case 'BAD_REQUEST':
                        $errors = PagSeguroRefundParser::readErrors($connection->getResponse());
                        $err = new PagSeguroServiceException($httpStatus, $errors);
                        LogPagSeguro::error("PagSeguroRefundService.createRefundRequest() - error " . $err->getOneLineMessage());
                        throw $err;
                        break;
                    default:
