    ...
    class PagSeguroAuthorizationSearchService
    {
        ...
        public static function searchByCode(PagSeguroCredentials $credentials, $authorizationCode)
        {
            ...
            try{
                ...
                $connection->get(self::buildSearchUrlByCode($connectionData, $authorizationCode), $connectionData->getServiceTimeout(), $connectionData->getCharset());
        ...
        public static function searchByNotificationCode(PagSeguroCredentials $credentials, $notificationCode)
        {
            ...
            try{
                $connection = new PagSeguroHttpConnection();
                $connection->get(self::buildSearchUrlByNotification($connectionData, $notificationCode), $connectionData->getServiceTimeout(), $connectionData->getCharset());
        ...
        public static function searchAuthorizations(PagSeguroCredentials $credentials, array $options = null)
        {
            ...
            try{
                $connection = new PagSeguroHttpConnection();
                $connection->get(self::buildSearchUrl($connectionData, $options), $connectionData->getServiceTimeout(), $connectionData->getCharset());
        ...
        private function searchReturn($connection, $code)
        {
            ...
            switch($httpStatus->getType()){
                case 'OK':
                    ...
                    LogPagSeguro::info(sprintf("PagSeguroAuthorizationSearchService.%s(code=$code) - end ", self::$logService) . $authorization->toString());
                    break;
                case 'BAD_REQUEST':
                    $errors = PagSeguroAuthorizationParser::readErrors($connection->getResponse());
                    $err = new PagSeguroServiceException($httpStatus, $errors);
                    LogPagSeguro::error(sprintf("PagSeguroAuthorizationSearchService.%s(code=$code) - error ", self::$logService) . $err->getOneLineMessage());
                    throw $err;
                    break;
                default:
