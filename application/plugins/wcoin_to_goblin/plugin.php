<?php
    ...
    class _plugin_wcoin_to_goblin extends controller implements pluginInterface
    {
        private $pluginaizer;
        private $vars = [];
        ...
        private function user_module()
        {
            ...
            if($this->pluginaizer->session->is_user()){
                ...
                //load template
                $this->load->view('plugins' . DS . $this->pluginaizer->get_plugin_class() . DS . 'views' . DS . $this->config->config_entry('main|template') . DS . 'view.wcoins_exchange', $this->vars);
            } else{
                ...
                if($this->vars['plugin_config'] != false && !empty($this->vars['plugin_config'])){
                    ...
                    if($this->vars['plugin_config']['active'] == 0){
                        ...
                        if(count($_POST) > 0){
                            ...
                            list($c, $c2) = explode('/', $this->vars['plugin_config']['ratio']);
                            $this->vars['times'] = floor($game_currency / $c);
                            $this->vars['reward'] = $this->vars['times'] * $c2;
                            ...
                            if(!$this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->check_connect_stat($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server'])))
                                ...
                                if($game_currency > $this->vars['plugin_config']['max_wcoins']){
                                    echo $this->pluginaizer->jsone(['error' => sprintf(__('Maximum Exchange At Once: %s %s'), $this->vars['plugin_config']['max_wcoins'], 'GoblinPoints')]);
                                } else{
                                    if($game_currency < $c){
                                        echo $this->pluginaizer->jsone(['error' => sprintf(__('Minimum Exchange: %s %s'), $c, 'GoblinPoints')]);
                                    } else{
                                        $status = $this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->get_user_wcoins_balance($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
                                        if($status < $game_currency){
                                            echo $this->pluginaizer->jsone(['error' => sprintf(__('You have insufficient amount of %s'), 'GoblinPoints')]);
                                            return;
                                        }
                                        $this->pluginaizer->Maccount->add_account_log('Exchanged GoblinPoints In Wcoins', -$game_currency, $this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
                                        $this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->remove_wcoins($game_currency, $this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
										$this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->add_goblinpoint($this->vars['reward'], $this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
										echo $this->pluginaizer->jsone(['success' => __('GoblinPoints exchanged successfully.')]);
                                    }
                                }
                            }
                        }
        ...
        public function install()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->pluginaizer->set_about()->add_plugin([
                    ...
                    'description' => 'Here you can exchange goblinpoints to wcoins.' //description which will see user
                ]);
                //create plugin config template
                $this->pluginaizer->create_config(['active' => 0, 'max_wcoins' => 1000, 'ratio' => "1/1"]);
                //check for errors
