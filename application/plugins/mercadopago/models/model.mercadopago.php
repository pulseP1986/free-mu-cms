<?php
    ...
    class Mmercadopago extends model
    {
        ...
        public function load_packages($status = false)
        {
            ...
            return $this->website->db('web')->query('SELECT id, package, reward, price, currency, orders, status, server, country_code FROM DmN_Donate_MercadoPago_Packages ' . $where . ' ORDER BY orders ASC')->fetch_all();
        }

        /**
         * Check if MercadoPago package exists
         *
         * @param int $id
         *
         *
        ...
        {
            $stmt = $this->website->db('web')->prepare('SELECT id, reward, price, currency, country_code FROM DmN_Donate_MercadoPago_Packages WHERE id = :id');
            $stmt->execute([':id' => $id]);
            return $stmt->fetch();
        }
         ...
         *
         */
        public function add_package($title, $price, $currency, $reward, $server, $country_code = null)
        {
            $max_orders = $this->website->db('web')->query('SELECT ISNULL(MAX(orders), 0) AS max_orders FROM DmN_Donate_MercadoPago_Packages')->fetch();
            $stmt = $this->website->db('web')->prepare('INSERT INTO DmN_Donate_MercadoPago_Packages (package, reward, price, currency, orders, status, server, country_code) VALUES (:title, :reward, :price, :currency, :count, 1, :server, :country_code)');
            $stmt->execute([':title' => $title, ':reward' => $reward, ':price' => $price, ':currency' => $currency, ':count' => $max_orders['max_orders'], ':server' => $server, ':country_code' => $country_code]);
            return $this->website->db('web')->last_insert_id();
        }

        /**
         * Edit existing MercadoPago package
         *
         * @param int $id
         * @param string $title
         * @param float $price
         ...
         *
         */
        public function edit_package($id, $title, $price, $currency, $reward, $server, $country_code = null)
        {
            $stmt = $this->website->db('web')->prepare('UPDATE DmN_Donate_MercadoPago_Packages SET package = :title, reward = :reward, price = :price, currency = :currency, server = :server, country_code = :country_code WHERE id = :id');
            $stmt->execute([':title' => $title, ':reward' => $reward, ':price' => $price, ':currency' => $currency, ':server' => $server, ':country_code' => $country_code, ':id' => $id]);
        }

        /**
         * Remove MercadoPago package
         *
         * @param int $id
         *
         *
        ...
        {
            $stmt = $this->website->db('web')->prepare('DELETE FROM DmN_Donate_MercadoPago_Packages WHERE id = :id');
            $stmt->execute([':id' => $id]);
        }

        /**
         * Enable / Disabled MercadoPago package
         *
         * @param int $id
         * @param int $status
         *
        ...
        {
            $stmt = $this->website->db('web')->prepare('UPDATE DmN_Donate_MercadoPago_Packages SET status = :status WHERE id = :id');
        ...
        public function save_order($orders)
        {
            foreach($orders as $key => $value){
                $id = explode('_', $value);
                $stmt = $this->website->db('web')->prepare('UPDATE DmN_Donate_MercadoPago_Packages SET orders = :order WHERE id = :id');
                $stmt->execute([':order' => $key, ':id' => end($id)]);
            }
        }

        /**
         * Load MercadoPago transactions for logs
         *
         * @param int $page
         * @param int $per_page
         * @param string $acc
        ...
        /**
         * Count total MercadoPago transactions for pagination
         *
         * @param string $acc
         * @param string $server
         *
        ...
        {
            $stmt = $this->website->db('web')->prepare('INSERT INTO DmN_Donate_MercadoPago_Orders (amount, currency, credits, account, server, hash) VALUES(:amount, :currency, :credits, :account, :server, :hash)');
            return $stmt->execute([':amount' => $price, ':currency' => $currency, ':credits' => $reward, ':account' => $user, ':server' => $server, ':hash' => $item]);
        }

        /**
         * Check if MercadoPago order exists
         *
         * @param string $item
         *
         *
        ...
        /**
         * Check if MercadoPago transaction already processed
         *
         * @param string $item
         *
         *
        ...
        /**
         * Insert MercadoPago transaction
         *
         * @param string $item
         *
         *
        ...
        {
            $stmt = $this->website->db('web')->prepare('INSERT INTO DmN_Donate_MercadoPago_Transactions (transaction_id, amount, currency, acc, server, credits, order_date, order_hash) VALUES (:trans_id, :gross, :currency, :account, :server, :credits, :time, :order_hash)');
