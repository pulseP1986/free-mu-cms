...
class Mpaghiper extends model{
	...
	public function check_order_number($item){
        $count = $this->website->db('web')->snumrows('SELECT COUNT(id) AS count FROM DmN_Donate_Paghiper_Orders where hash = \'' . $this->website->db('web')->sanitize_var($item) . '\'');
        if ($count == 1){
            $this->order_details = $this->website->db('web')->query('SELECT amount, currency, account, server, credits, hash FROM DmN_Donate_Paghiper_Orders where hash = \'' . $this->website->db('web')->sanitize_var($item) . '\'')->fetch();
            return true;
        } 
		else{
            return false;
	...
	public function check_completed_transaction($item){
        return $this->website->db('web')->query('SELECT amount, currency, acc, server, credits, status FROM DmN_Donate_Paghiper_Transactions where order_hash = \'' . $this->website->db('web')->sanitize_var($item) . '\'')->fetch();
    }
