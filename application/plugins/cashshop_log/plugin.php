<?php
    class _plugin_cashshop_log extends controller implements pluginInterface
    {
        private $pluginaizer;
        private $vars = [];
            ...
            if($this->pluginaizer->session->is_user()){
                ...
                if($this->vars['plugin_config'] != false && !empty($this->vars['plugin_config'])){
                    ...
                    if($this->vars['plugin_config']['active'] == 0){
						...
						$this->vars['logs'] = $this->{'M'.$this->pluginaizer->get_plugin_class()}->getLogs($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
  
						if($this->vars['logs'] === false){
							$this->vars['config_not_found'] = __('T_InGameShop_Log table not found.');
						}
					}
                } 
				else{
        ...
        public function install()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->pluginaizer->set_about()->add_plugin([
                    ...
                    'description' => 'View CashShop Logs.' //description which will see user
                ]);
