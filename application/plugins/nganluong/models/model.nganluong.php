<?php
    ...
    class Mnganluong extends model
    {
        ...
        {
            $count = $this->website->db('web')->snumrows('SELECT COUNT(id) AS count FROM DmN_Donate_NganLuong_Orders where hash = '.$this->website->db('web')->escape($item).'');
            if($count == 1){
                $this->order_details = $this->website->db('web')->query('SELECT amount, currency, account, server, credits FROM DmN_Donate_NganLuong_Orders where hash = '.$this->website->db('web')->escape($item).'')->fetch();
                return true;
            } else{
                return false;
            }
        ...
        {
            $count = $this->website->db('web')->snumrows('SELECT COUNT(id) AS count FROM DmN_Donate_NganLuong_Transactions where order_hash = '.$this->website->db('web')->escape($item).'');
            if($count > 0){
                return true;
            }
            return false;
