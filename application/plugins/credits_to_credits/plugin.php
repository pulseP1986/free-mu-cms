<?php
    ...
    class _plugin_credits_to_credits extends controller implements pluginInterface
    {
        private $pluginaizer;
        private $vars = [];
        ...
        private function user_module()
        {
            ...
            if($this->pluginaizer->session->is_user()){
                ...
                //load template
                $this->load->view('plugins' . DS . $this->pluginaizer->get_plugin_class() . DS . 'views' . DS . $this->config->config_entry('main|template') . DS . 'view.credits_exchange', $this->vars);
            } else{
                            ...
                            if(!$this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->check_connect_stat($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server'])))
                                ...
                                if($game_currency > $this->vars['plugin_config']['max_credits']){
                                    ...
                                    if($game_currency < $c){
                                        ...
                                        $this->pluginaizer->Maccount->add_account_log('Exchanged ' . $this->pluginaizer->website->translate_credits($this->vars['plugin_config']['exchange_type'], $this->pluginaizer->session->userdata(['user' => 'server'])) . ' In Credits Exchange', -$game_currency, $this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
                                        ...
                                        $type = ($this->vars['plugin_config']['exchange_type'] == 1) ? 2 : 1;
                                        $this->pluginaizer->website->add_credits($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']), $this->vars['reward'], $type, false, $this->pluginaizer->session->userdata(['user' => 'id']));
                                        echo $this->pluginaizer->jsone(['success' => __('Credits exchanged successfully.')]);
                                    }
                                }
                            }
                        }
        ...
        public function install()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->pluginaizer->set_about()->add_plugin([
                    ...
                    'description' => 'Here you can exchange web/game currency for other web/game currency.' //description which will see user
                ]);
                //create plugin config template
                $this->pluginaizer->create_config(['active' => 0, 'max_credits' => 1000, 'ratio' => "1/1", 'exchange_type' => 1]);
                //check for errors
