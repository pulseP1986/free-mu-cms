<?php
    ...
    class Mgift_code extends model
    {
		...
		public function checkCode($code){
			return $this->website->db('web')->query('SELECT id, code, expires, max_uses_total, uses_left, max_uses_by_user, max_uses_by_char, min_lvl, min_mlvl, min_res, min_gres, code_type, code_reward_currency, code_reward_vip, code_reward_items, char_class, server FROM DmN_Gift_Codes WHERE code = '.$this->website->db('web')->escape($code).'')->fetch();
		}

		public function checkCodeWheel($code, $server){
			if($this->website->db('web')->check_if_table_exists('DmN_Wheel_Of_Fotune_Rewards')){
				return $this->website->db('web')->query('SELECT id, reward_id, account, is_claimed FROM DmN_Wheel_Of_Fotune_Rewards WHERE code = '.$this->website->db('web')->escape($code).' AND server = '.$this->website->db('web')->escape($server).'')->fetch();
			}
			return false;
		}
		...
		public function deleteCode($id){
			$this->website->db('web')->query('DELETE FROM DmN_Gift_Codes WHERE id = '.$this->website->db('web')->escape($id).'');
		}
		...
		public function checkUsesByCharacter($coupon, $id, $user, $server){
			return $this->website->db('web')->snumrows('SELECT COUNT(id) AS count FROM DmN_Gift_Codes_Log WHERE code = '.$this->website->db('web')->escape($coupon).' AND character = '.$this->website->db('web')->escape($id).' AND account = '.$this->website->db('web')->escape($user).' AND server = '.$this->website->db('web')->escape($server).'');
		}
		
		public function checkUsesByAccount($coupon, $user, $server){
			return $this->website->db('web')->snumrows('SELECT COUNT(id) AS count FROM DmN_Gift_Codes_Log WHERE code = '.$this->website->db('web')->escape($coupon).' AND account = '.$this->website->db('web')->escape($user).' AND server = '.$this->website->db('web')->escape($server).'');
		}
		...
		public function setUsesLeft($coupon){
			$this->website->db('web')->query('UPDATE DmN_Gift_Codes SET uses_left = uses_left - 1 WHERE  code = '.$this->website->db('web')->escape($coupon).'');
		}
		
		public function load_logs($page = 1, $per_page = 25, $acc = '', $server = 'All'){
            if(($acc == '' || $acc == '-') && $server == 'All')
                $items = $this->website->db('web')->query('SELECT Top ' . $this->website->db('web')->escape($per_page) . '  id, code, account, server, character, date_used FROM DmN_Gift_Codes_Log WHERE id Not IN (SELECT Top ' . $this->website->db('web')->escape($per_page * ($page - 1)) . ' id FROM DmN_Gift_Codes_Log ORDER BY id DESC) ORDER BY id DESC'); 
			else{
                if(($acc != '' && $acc != '-') && $server == 'All')
                    $items = $this->website->db('web')->query('SELECT Top ' . $this->website->db('web')->escape($per_page) . ' id, code, account, server, character, date_used FROM DmN_Gift_Codes_Log WHERE account like \'%' . $this->website->db('web')->escape($acc) . '%\' AND id Not IN (SELECT Top ' . $this->website->db('web')->escape($per_page * ($page - 1)) . ' id FROM DmN_Gift_Codes_Log WHERE account like \'%' . $this->website->db('web')->escape($acc) . '%\' ORDER BY id DESC) ORDER BY id DESC'); else
				$items = $this->website->db('web')->query('SELECT Top ' . $this->website->db('web')->escape($per_page) . ' id, code, account, server, character, date_used FROM DmN_Gift_Codes_Log WHERE account like \'%' . $this->website->db('web')->escape($acc) . '%\' AND server = '.$this->website->db('web')->escape($server).' AND id Not IN (SELECT Top ' . $this->website->db('web')->escape($per_page * ($page - 1)) . ' id DmN_Gift_Codes_Log WHERE account like \'%' . $this->website->db('web')->escape($acc) . '%\' AND server = '.$this->website->db('web')->escape($server).' ORDER BY id DESC) ORDER BY id DESC');
            }
			$logs = [];
		...
		public function add_wcoins($amount, $user, $server, $id, $config = []){
		...
		public function add_goblinpoints($amount, $user, $server, $id, $config = []){
            $acc = (in_array($config['identifier_column'], ['MemberGuid', 'memb_guid'])) ? $id : $user;
            $stmt = $this->website->db($config['db'], $server)->prepare('UPDATE ' . $config['table'] . ' SET ' . $config['column'] . ' = ' . $config['column'] . ' + :wcoins WHERE ' . $config['identifier_column'] . ' = :account');
            $stmt->execute([':wcoins' => $amount, ':account' => $acc]);
            if($stmt->rows_affected() == 0){
