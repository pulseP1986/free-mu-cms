...
class Mgerencianet extends model{
	...
	public function load_packages($status = false){
        ...
        return $this->website->db('web')->query('SELECT id, package, reward, price, currency, orders, status, server FROM DmN_Donate_Gerencianet_Packages '.$where.' ORDER BY orders ASC')->fetch_all();
    }
	
	/**
     * Check if Gerencianet package exists
	 *
	 * @param int $id
     *
     *
	...
	public function check_package($id){
        $stmt = $this->website->db('web')->prepare('SELECT id, reward, price, currency FROM DmN_Donate_Gerencianet_Packages WHERE id = :id');
        $stmt->execute([':id' => $id]);
        return $stmt->fetch();
    }
	
	/**
     * Add Gerencianet package
	 *
	 * @param string $title
	 * @param float $price
	 * @param int $currency
	...
	/**
     * Edit existing Gerencianet package
	 *
	 * @param int $id
	 * @param string $title
	 * @param float $price
	...
	public function delete_package($id){
        $stmt = $this->website->db('web')->prepare('DELETE FROM DmN_Donate_Gerencianet_Packages WHERE id = :id');
        $stmt->execute([':id' => $id]);
    }
	
	/**
     * Enable / Disabled Gerencianet package
	 *
	 * @param int $id
	 * @param int $status
     *
	...
	public function change_status($id, $status){
        $stmt = $this->website->db('web')->prepare('UPDATE DmN_Donate_Gerencianet_Packages SET status = :status WHERE id = :id');
        $stmt->execute([
			':status' => $status, 
			':id' => $id
		]);
	...
	/**
     * Save Gerencianet package order
     *
	 * @param array $orders
     *
     */
	...
	public function save_order($orders){
        foreach ($orders as $key => $value){
            $id = explode('_', $value);
            $stmt = $this->website->db('web')->prepare('UPDATE DmN_Donate_Gerencianet_Packages SET orders = :order WHERE id = :id');
            $stmt->execute([
				':order' => $key, 
				':id' => end($id)
			]);
	...
	/**
     * Load Gerencianet transactions for logs
	 *
	 * @param int $page
	 * @param int $per_page
	 * @param string $acc
	...
	/**
     * Count total Gerencianet transactions for pagination
	 *
	 * @param string $acc
	 * @param string $server
     *
	...
	public function count_total_transactions($acc = '', $server = 'All'){
        ...
        $count = $this->website->db('web')->snumrows('SELECT COUNT(acc) AS count FROM DmN_Donate_Gerencianet_Transactions ' . $sql . '');
        return $count;
    }
	
	/**
     * Insert Gerencianet order
	 *
	 * @param float $price
	 * @param int $currency
	 * @param int $reward
	...
	public function insert_order($price, $currency, $reward, $item, $user, $server){
        $stmt = $this->website->db('web')->prepare('INSERT INTO DmN_Donate_Gerencianet_Orders (amount, currency, credits, account, server, hash) VALUES(:amount, :currency, :credits, :account, :server, :hash)');
        return $stmt->execute([
			':amount' => $price, 
			':currency' => $currency, 
			':credits' => $reward, 
	...
	/**
     * Check if Gerencianet order exists
	 *
	 * @param string $item
     *
     *
	...
	/**
     * Check if Gerencianet transaction already processed
	 *
	 * @param string $item
     *
     *
	...
	/**
     * insert Gerencianet transaction
	 *
	 * @param string $item
     *
     *
	...
	public function insert_transaction_status($id, $item, $status){
        $stmt = $this->website->db('web')->prepare('INSERT INTO DmN_Donate_Gerencianet_Transactions (transaction_id, amount, currency, acc, server, credits, order_date, order_hash, status) VALUES (:trans_id, :gross, :currency, :account, :server, :credits, :time, :order_hash, :status)');
        return $stmt->execute([
			':trans_id' => $id, 
	...
	public function update_transaction_status($item, $status){
        $stmt = $this->website->db('web')->prepare('UPDATE DmN_Donate_Gerencianet_Transactions SET status = :status WHERE order_hash = :order_hash');
        return $stmt->execute([
			':status' => $status,
			':order_hash' => $item
		]);
