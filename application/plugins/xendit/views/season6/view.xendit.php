							...
							foreach($packages_xendit as $packages){
								echo '<ul id="paypal-options">
										<li>
											...
											<button class="right custom_button" id="buy_xendit_' . $packages['id'] . '" style="margin-top: 8px;" value="buy_payssion_' . $packages['id'] . '">' . __('Buy Now') . '</button>
										</li>
								</ul>';
							}
						} 
						else{
							echo '<div class="i_note">' . __('No Xendit Packages Found.') . '</div>';
						}
					}
					?>
				</div>
