			...
			<div class="row">
				<div class="col-12">   
					...
					$(document).ready(function(){
						$('button[id^="buy_xendit_"]').on('click', function () {
							xendit.checkout($(this).attr('id').split('_')[2]);
						});
					});
					</script>
					<?php
						if(isset($error)){
							...
							if(!empty($packages_xendit)){
								foreach($packages_xendit as $packages){
									echo '<ul id="paypal-options">
											<li>
												...
												<button class="float-right btn btn-primary" id="buy_xendit_' . $packages['id'] . '" value="buy_payssion_' . $packages['id'] . '">' . __('Buy Now') . '</button>
											</li>
									</ul>';
								}
							} 
							else{
								echo '<div class="alert alert-primary" role="alert">' . __('No Xendit Packages Found.') . '</div>';
							}
						}
					?>
					</div>
