		...
		<div class="dmn-page-content">
			<div class="row">
				<div class="col-12">   
					...
					var payssion = new payssion();
					payssion.setUrl('<?php echo $this->config->base_url . $this->request->get_controller();?>');
					$(document).ready(function(){
						$('button[id^="buy_payssion_"]').on('click', function () {
							payssion.checkout($(this).attr('id').split('_')[2]);
						});
					});
					</script>
					<?php
						if(isset($error)){
							...
							if(!empty($packages_payssion)){
								foreach($packages_payssion as $packages){
									echo '<ul id="paypal-options">
											<li>
												...
												<button class="float-right btn btn-primary" id="buy_payssion_' . $packages['id'] . '" value="buy_payssion_' . $packages['id'] . '">' . __('Buy Now') . '</button>
											</li>
									</ul>';
								}
							} 
							else{
								echo '<div class="alert alert-primary" role="alert">' . __('No Payssion Packages Found.') . '</div>';
							}
						}
					?>
					</div>
