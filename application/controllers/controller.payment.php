                    ...
                    if($this->Mdonate->post_back_paypal()){
                        ...
                        if($this->Mdonate->validate_paypal_payment()){
							...
							if(mb_strpos($_POST['item_number'], 'bp-') !== false){
								...
								$this->Mdonate->add_account_log('Purchased '.$type.' For ' . BPASS_CURRENCY, -$this->Mdonate->order_details['amount'], $this->Mdonate->order_details['account'], $this->Mdonate->order_details['server']);
								...
								if(mb_strpos($_POST['item_number'], 'mk-') !== false){
									...
									$this->Mdonate->add_account_log('Purchased Mystery Key For ' . WHEEL_KEYS_CURRENCY, -$this->Mdonate->order_details['amount'], $this->Mdonate->order_details['account'], $this->Mdonate->order_details['server']);
								}
								else{
                        ...
                        if(!$this->Mdonate->verifiedTransaction($paycall_unique, $this->config->values('donation_config', [$this->Mdonate->order_details['server'], 'paycall', 'business_code']), $this->config->values('donation_config', [$this->Mdonate->order_details['server'], 'paycall', 'sandbox']))){
                            ...
                            if($this->Mdonate->validate_paycall_payment($paycall_unique, $custom, $total_amount)){
								...
								if($this->config->values('referral_config', 'reward_on_donation') > 0){
									...
									if($ref != false){
										$ref_reward = floor(($this->config->values('referral_config', 'reward_on_donation') / 100) * $this->Mdonate->order_details['credits']);
										$this->Mdonate->reward_user($ref, $this->Mdonate->order_details['server'], $ref_reward, $this->config->values('donation_config', [$this->Mdonate->order_details['server'], 'paycall', 'reward_type']), $this->Mdonate->get_guid($ref));
										$this->Mdonate->add_account_log('Friend donation bonus ' . $this->website->translate_credits($this->config->values('donation_config', [$this->Mdonate->order_details['server'], 'paycall', 'reward_type']), $this->Mdonate->order_details['server']) . '', $ref_reward, $ref, $this->Mdonate->order_details['server']);
									}
								}
										...
										if(!$this->Mdonate->check_reference($acc_serv[0], $acc_serv[1], $_GET['ref'])){
											...
											if($this->config->values('referral_config', 'reward_on_donation') > 0){
												...
												if($ref != false){
													...
													$this->Mdonate->reward_user($ref, $this->Mdonate->order_details['server'], $ref_reward, $this->vars['donation_config']['reward_type'], $this->Mdonate->get_guid($ref));
													$this->Mdonate->add_account_log('Friend donation bonus ' . $this->website->translate_credits($this->vars['donation_config']['reward_type'], $acc_serv[1]) . '', $ref_reward, $ref, $acc_serv[1]);
												}
											}
											if($this->config->values('email_config', 'donate_email_user') == 1){
                        ...
                        if($this->Mdonate->check_existing_cuenta_digital_transaction($code)){
							...
							if($this->config->values('referral_config', 'reward_on_donation') > 0){
								...
								if($ref != false){
									...
									$this->Mdonate->reward_user($ref, $this->Mdonate->order_details['server'], $ref_reward, $this->config->values('donation_config', [$this->vars['order_data']['server'], 'cuenta_digital', 'reward_type']), $this->Mdonate->get_guid($ref));
									$this->Mdonate->add_account_log('Friend donation bonus ' . $this->website->translate_credits($this->config->values('donation_config', [$this->vars['order_data']['server'], 'cuenta_digital', 'reward_type']), $this->vars['order_data']['server']) . '', $ref_reward, $ref, $this->vars['order_data']['server']);
								}
							}
                            ...
                            if($this->vars['order_data'] != false){
                                if($this->Mdonate->check_existing_pagseguro_transaction($notificationCode)){
									...
									if($this->config->values('referral_config', 'reward_on_donation') > 0){
										...
										if($ref != false){
											$ref_reward = floor(($this->config->values('referral_config', 'reward_on_donation') / 100) * $this->vars['order_data']['amount']);
											$this->Mdonate->reward_user($ref, $this->Mdonate->order_details['server'], $ref_reward, $this->config->values('donation_config', [$this->vars['order_data']['server'], 'pagseguro', 'reward_type']), $this->Mdonate->get_guid($ref));
											$this->Mdonate->add_account_log('Friend donation bonus ' . $this->website->translate_credits($this->config->values('donation_config', [$this->vars['order_data']['server'], 'pagseguro', 'reward_type']), $this->vars['order_data']['server']) . '', $ref_reward, $ref, $this->vars['order_data']['server']);
										}
									}
